...
# Load Data
train_fours, test_fours = filter_digits(chosen_digit)
train_size = train_fours.shape[0]
num_batches = train_size // batch_size

for i in range(25):
    plt.subplot(5, 5, 1 + i)
    plt.axis('off')
    plt.imshow(train_fours[i, :, :, 0], cmap='gray')
...
for epoch in range(epochs):
    ...
    for i in range(num_batches):
        ...
        train_mixed_fours, train_mixed_labels = grab_discriminator_samples(train_fours, batch_size, i, permutation,
                                                                           model)
        disc_mets = discriminator.train_on_batch(train_mixed_fours, train_mixed_labels)
    ...
    disc_mets = discriminator.test_on_batch(test_fours[range(0, 25), :, :, :], np.ones([25]))
